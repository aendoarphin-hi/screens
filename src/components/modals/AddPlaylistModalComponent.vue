            ...
            <div class="pe-3" style="flex: 1 1 0; min-width: 0;">
              <div class="vstack gap-2">
                ...
                <input disabled type="text" class="form-control form-control-sm" :value="createdByName"
                  title="Set automatically to the logged-in user">
                <small class="text-muted text-center" style="font-size: 10px">
                  The playlist is automatically assigned to the logged-in user and cannot be changed.
                </small>
              </div>
            </div>
...
export default {
    ...
    async submit() {
      try {
        ...
        const body = {
          ...
          description: this.form.description && this.form.description.trim().length > 0
            ? this.form.description.trim()
            : null,
          content: this.contentQueue.length > 0 ? this.contentQueue.join(",") : null,
        ...
        await this.$axios.post(this.$api + "activity?new", {
          ...
          entity_json: JSON.stringify(body)
        })
        this.$modal.hide('add-playlist-modal');
        this.toast.show("Playlist Created", "The playlist has been successfully created.", "bg-success-subtle text-success-emphasis");
        this.clearChanges();
        this.$emit('created');
      } catch (error) {
        console.error(error);
  ...
  computed: {
    createdByName() {
      return this.store?.authenticated?.name ?? '';
    },
    /**
